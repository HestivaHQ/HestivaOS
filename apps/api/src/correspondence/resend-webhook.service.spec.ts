...
describe('ResendWebhookService', () => {
  ...
  afterEach(() => {
    if (originalSecret === undefined) delete process.env.RESEND_WEBHOOK_SIGNING_SECRET;
    else process.env.RESEND_WEBHOOK_SIGNING_SECRET = originalSecret;
  ...
  it('rejects stale signed webhook timestamps', async () => {
    ...
    const old = String(Math.floor(Date.now() / 1000) - 601);

    await expect(service.ingest(raw, signed(raw, secret, 'event_old', old))).rejects.toBeInstanceOf(BadRequestException);
  });
});
