...
export type QuoteOperationalCostCandidate = Partial<QuoteOperationalCostSnapshot> & { provenance?: QuoteOperationalCostProvenance };

export type QuoteOperationalCostResolution =
  | { kind: 'READY'; costs: QuoteOperationalCostSnapshot; provenance: QuoteOperationalCostProvenance }
  | { kind: 'NEEDS_ATTENTION'; missing: QuoteOperationalCostComponent[]; invalid: QuoteOperationalCostComponent[]; provenance: QuoteOperationalCostProvenance };
...
export function validateOperationalCostCandidate(candidate: QuoteOperationalCostCandidate | null | undefined): QuoteOperationalCostResolution {
  ...
  for (const component of QUOTE_OPERATIONAL_COST_COMPONENTS) {
    ...
    if (value === undefined || value === null) missing.push(component);
    else if (!validMinorUnitAmount(value)) invalid.push(component);
  }
  if (missing.length || invalid.length) return { kind: 'NEEDS_ATTENTION', missing, invalid, provenance };
  return {
    kind: 'READY',
    costs: {
...
export async function resolveQuoteOperationalCosts(provider: QuoteOperationalCostProvider, submission: QuotePricingSubmission): Promise<QuoteOperationalCostResolution> {
