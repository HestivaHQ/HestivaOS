...
import type { QuotePricingReviewService } from './quote-pricing-review.service';
...
describe('QuoteReviewController authorization and delegation', () => {
  it('is ADMIN-only and does not bypass the global authentication guard', () => {
    ...
    for (const method of ['findAll', 'findOne', 'preflight', 'reviewPricing', 'decline', 'recordResolution', 'accept'] as const) {
  ...
  it('delegates list and detail review to the internal service', async () => {
    ...
    const pricingReview = { review: jest.fn() } as unknown as QuotePricingReviewService;
    const controller = new QuoteReviewController(service, pricingReview);
