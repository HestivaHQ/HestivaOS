...
const inboundEvent = {
  contractVersion: '1.0', channel: 'WHATSAPP', provider: 'meta', providerEventId: 'event-1',
  identity: { providerIdentityId: 'identity-1' }, occurredAt: '2026-08-21T16:00:00.000Z', receivedAt: '2026-08-21T16:00:01.000Z',
  kind: 'TEXT', text: 'Hello',
};
const automationAuthority = () => ({ automationAuthority: jest.fn(async () => ({ controlVersion: 0 })) });
...
describe('webhook trusted identity and Quote orchestration', () => {
  it('runs trusted identity and guided Quote orchestration after durable WhatsApp inbound persistence when Flow does not own the message', async () => {
    ...
    const messaging = { persistInbound: jest.fn(async () => ({ id: 'message-1', conversationId: 'conversation-1' })), persistWhatsAppStatus: jest.fn() };
    ...
    const controller = new WhatsAppWebhookController(adapter as any, messaging as any, inboundMedia as any, customerLinking as any, quoteOrchestrator as any, quoteFlowInbound as any, automationAuthority() as any);
    ...
    expect(quoteFlowInbound.handleInbound).toHaveBeenCalledWith('message-1', 0);
  ...
  it('does not feed Flow-owned WhatsApp inbound into the guided collector', async () => {
    ...
    expect(quoteFlowInbound.handleInbound).toHaveBeenCalledWith('message-flow', 0);
  ...
  it('persists WhatsApp inbound and secured media but suppresses Flow and Quote automation during takeover', async () => {
    ...
    const control = { automationAuthority: jest.fn(async () => null) };
