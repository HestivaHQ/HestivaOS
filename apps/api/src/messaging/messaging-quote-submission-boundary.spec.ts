...
describe('prepareMessagingQuoteSubmission', () => {
  ...
  it('does not allow submission while human review is required', () => {
    const result = prepareMessagingQuoteSubmission({ draft: validDraft(), customerConfirmed: true, humanReviewRequired: true });
  ...
  it('allows a complete confirmed Post-Event draft through the channel-neutral business boundary', () => {
    ...
    expect(prepareMessagingQuoteSubmission({ draft, customerConfirmed: true })).toEqual({ kind: 'READY', draft });
  });
  ...
  it('rejects Post-Event facts attached to a different primary service', () => {
    const draft = validPostEventDraft();
    draft.request.primaryService = { websiteValue: 'Regular Home Cleaning', canonicalService: 'Regular Home Cleaning' };
