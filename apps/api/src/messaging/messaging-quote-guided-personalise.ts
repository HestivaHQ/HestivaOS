...
import {
  applyMessagingGuidedVisitHouseholdAnswer,
  nextMessagingGuidedVisitHouseholdQuestion,
  type MessagingGuidedVisitHouseholdQuestion,
} from './messaging-quote-guided-visit-household';

export type MessagingGuidedPersonaliseQuestionId =
  | 'ADD_ONS'
...
export type MessagingGuidedPersonaliseQuestion =
  | { id: MessagingGuidedPersonaliseQuestionId; text: string }
  | MessagingGuidedVisitHouseholdQuestion;

type AddOn = { websiteValue: string; canonicalService: string; quantity: number };

const ADD_ONS = {
  '1': { websiteValue: 'Inside oven', canonicalService: 'Inside Oven Cleaning' },
  '2': { websiteValue: 'Inside fridge', canonicalService: 'Inside Fridge Cleaning' },
  '3': { websiteValue: 'Inside cupboards', canonicalService: 'Interior Cupboard Cleaning' },
  '4': { websiteValue: 'Interior windows', canonicalService: 'Interior Window Cleaning' },
  '5': { websiteValue: 'Bed making', canonicalService: 'Bed Making' },
  '6': { websiteValue: 'Linen change', canonicalService: 'Linen Change' },
  '7': { websiteValue: 'Balcony / Patio Cleaning', canonicalService: 'Balcony / Patio Cleaning' },
  '8': { websiteValue: 'Garage sweep', canonicalService: 'Garage Sweeping' },
  '9': { websiteValue: 'Extra bathroom', canonicalService: 'Extra Bathroom Cleaning' },
  '10': { websiteValue: 'Extra refrigerator', canonicalService: 'Extra Refrigerator' },
  '11': { websiteValue: 'Pet-hair treatment', canonicalService: 'Pet-Hair Treatment' },
} as const;

const LAUNDRY_FACILITIES: Record<string, LaundryFacilities> = {
  '1': 'WASHER_DRYER',
  '2': 'WASHER_LINE',
};
...
export function nextMessagingGuidedPersonaliseQuestion(
  ...
  if (laundry) {
    ...
    if (wantsLaundry && !laundry.facilities) {
      return { id: 'LAUNDRY_FACILITIES', text: 'Laundry requires a working washing machine at the property. Which setup is available?\n1. Washing machine and dryer\n2. Washing machine and clothes line / drying rack\nReply with the number only.' };
    }
...
export type MessagingGuidedPersonaliseAnswer =
  | { kind: 'ACCEPTED'; patch: MessagingQuoteDraftProgress }
  | { kind: 'INVALID'; question: MessagingGuidedPersonaliseQuestion }
  | { kind: 'COMPLETE' };

export function applyMessagingGuidedPersonaliseAnswer(
  draft: MessagingQuoteDraftProgress,
  rawText: string | null | undefined,
): MessagingGuidedPersonaliseAnswer {
  const question = nextMessagingGuidedPersonaliseQuestion(draft);
  ...
  if (![
    ...
    return applyMessagingGuidedVisitHouseholdAnswer(draft, rawText);
  }
  ...
  if (question.id === 'ADD_ONS') {
    ...
    if ((wantsLaundry || wantsIroning) && !isLaundryEligiblePrimary(primary)) {
      ...
      patch: {
        request: {
          ...
          ...((wantsLaundry || wantsIroning)
            ? { laundry: { ...(wantsLaundry ? { laundryLoads: 0 } : {}), ...(wantsIroning ? { ironingLoads: 0 } : {}) } }
            : {}),
        },
      },
    };
  ...
  if (question.id === 'EXTRA_REFRIGERATOR_QUANTITY' || question.id === 'BALCONY_PATIO_QUANTITY') {
    const quantity = positiveInteger(text);
    if (!quantity) return { kind: 'INVALID', question };
    const target = question.id === 'EXTRA_REFRIGERATOR_QUANTITY'
      ? 'Extra Refrigerator'
    ...
    return {
      ...
      patch: {
        request: {
          addOns: addOns.map((item) => item.canonicalService === target ? { ...item, quantity } : item),
        },
      },
    };
  }
  ...
  if (question.id === 'LAUNDRY_FACILITIES') {
    const facilities = LAUNDRY_FACILITIES[text];
    if (!facilities) return { kind: 'INVALID', question };
    return {
      kind: 'ACCEPTED',
      patch: { request: { laundry: { ...laundryProgress(request), facilities } } },
    };
  }

  const quantity = positiveInteger(text);
  if (!quantity) return { kind: 'INVALID', question };
  if (question.id === 'LAUNDRY_LOADS') {
    return {
      kind: 'ACCEPTED',
      patch: { request: { laundry: { ...laundryProgress(request), laundryLoads: quantity } } },
    };
  }
  return {
    kind: 'ACCEPTED',
    patch: { request: { laundry: { ...laundryProgress(request), ironingLoads: quantity } } },
  };
}
