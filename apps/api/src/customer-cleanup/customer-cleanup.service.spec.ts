...
function harness(customer: { contactName: string | null; name: string } | null = { contactName: 'Customer A', name: 'Legacy A' }) {
  const tx: any = {
    ...
    recurringServiceAgreement: { count: jest.fn().mockResolvedValue(1 as never), deleteMany: jest.fn().mockResolvedValue({ count: 1 } as never) },
...
describe('CustomerCleanupService', () => {
  it('returns authoritative owned-record impact counts and Contact name', async () => {
    ...
    await expect(service.impact('customer-a')).resolves.toMatchObject({ customerName: 'Customer A', customer: 1, properties: 2, recurringAgreements: 1, workOrders: 1, activities: 3, checklistItems: 4, photos: 2, signOffs: 1, shiftsToDetach: 1 });
  });
  ...
  it('deletes only owned rows, detaches shifts, and reports actual transaction counts', async () => {
    const { service, tx } = harness();
    await expect(service.remove('admin', 'customer-a', 'Customer A')).resolves.toMatchObject({ customerDeleted: 1, propertiesDeleted: 2, recurringAgreementsDeleted: 1, workOrdersDeleted: 1, activitiesDeleted: 3, checklistItemsDeleted: 4, photosDeleted: 2, signOffsDeleted: 1, shiftsDetached: 1, storageObjectsDeleted: false, possibleOrphanedStorage: true });
