...
export function TechnicianJobView({ workOrderId }: { workOrderId: string }) {
  ...
  const physicalFacts = [workOrder.property.propertyTypeOption?.label, workOrder.property.bedrooms && `${readableStatus(workOrder.property.bedrooms)} bedrooms`, workOrder.property.bathrooms && `${readableStatus(workOrder.property.bathrooms)} bathrooms`, workOrder.property.storeys && `${readableStatus(workOrder.property.storeys)} storeys`, workOrder.property.unitFloor && `${readableStatus(workOrder.property.unitFloor)} floor`, workOrder.property.outdoorArea && `Outdoor: ${readableStatus(workOrder.property.outdoorArea)}`, workOrder.property.estateClassification && readableStatus(workOrder.property.estateClassification), !workOrder.property.estateClassification && workOrder.property.isEstateOrComplex ? 'Estate/complex (legacy classification)' : null].filter(Boolean);
  const operationalNotes = [workOrder.property.hasPets ? (workOrder.property.petNotes || 'Pets present') : null, workOrder.property.offLimitsNotes, workOrder.property.fragileItemNotes, workOrder.property.productRestrictionNotes, workOrder.property.allergyNotes].filter(Boolean);
  return <><header className="pageHeader"><div><p className="eyebrow">Technician job</p><h2>{workOrderReference(workOrder)}</h2><p>{workOrderDisplayLabel(workOrder)}</p></div><Link href="/work-orders">Back to work orders</Link></header>{error ? <p className="errorBanner">{error}</p> : null}<div className="resourceGrid"><section className="panel resourceForm"><div className="panelHeader"><h3>Job details</h3><span className="statusPill">{readableStatus(workOrder.status)}</span></div><p><strong>Frequency</strong><br />{workOrderFrequencyLabel(workOrder)}</p>{workOrder.homeCondition ? <p><strong>Home condition</strong><br />{homeConditionLabels[workOrder.homeCondition]}</p> : null}{workOrder.addOns.length ? <p><strong>Add-ons</strong><br />{workOrder.addOns.map((item) => item.service.name).join(', ')}</p> : null}<p><strong>Scheduled</strong><br />{workOrder.scheduledAt ? new Intl.DateTimeFormat('en-ZA', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(workOrder.scheduledAt)) : 'Not scheduled'}</p><p><strong>Address</strong><br />{address}</p>{physicalFacts.length ? <p><strong>Property</strong><br />{physicalFacts.join(' · ')}</p> : null}{workOrder.property.accessNotes || workOrder.property.parkingNotes || workOrder.property.requiresGateSecurityAccess ? <p><strong>Access instructions</strong><br />{[workOrder.property.requiresGateSecurityAccess ? 'Gate/security access required' : null, workOrder.property.accessNotes, workOrder.property.parkingNotes].filter(Boolean).join(' · ')}</p> : null}{operationalNotes.length ? <p><strong>Household & care</strong><br />{operationalNotes.join(' · ')}</p> : null}{workOrder.description ? <p><strong>Job notes</strong><br />{workOrder.description}</p> : null}<p><strong>Assigned technicians</strong><br />{workOrder.assignedTechnicians.length ? workOrder.assignedTechnicians.map((item) => `${item.technician.firstName} ${item.technician.lastName}`).join(', ') : 'Unassigned'}{workOrder.crew ? <> <br /><strong>Crew:</strong> {workOrder.crew.name}</> : null}</p>{action ? <button className="primaryButton" disabled={saving} onClick={() => void changeStatus(action.status)}>{saving ? 'Updating…' : action.label}</button> : null}</section><section className="panel"><div className="panelHeader"><h3>Cleaning checklist</h3><span>{completedCount}/{checklist.length}</span></div><div className="dataList">{checklist.map((item) => <article className="dataRow" key={item.id}><div><strong>{item.description}</strong><p>{readableStatus(item.status)}</p></div><select value={item.status} onChange={(event) => void updateChecklist(item, event.target.value as WorkOrderChecklistItem['status'])}><option value="PENDING">Pending</option><option value="COMPLETED">Completed</option><option value="NOT_APPLICABLE">Not applicable</option></select></article>)}{!checklist.length ? <div className="emptyState"><strong>No checklist items</strong><p>The office can add tasks from the work-order screen.</p></div> : null}</div></section>{photoSection('Before photos', 'BEFORE', beforePhotos)}{photoSection('After photos', 'AFTER', afterPhotos)}<CustomerSignOff workOrderId={workOrderId} status={workOrder.status} /></div></>;
}
