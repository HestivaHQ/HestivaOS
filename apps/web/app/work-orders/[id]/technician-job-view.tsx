...
async function compressPhoto(file: File): Promise<File> {
  ...
  if (!context) { bitmap.close(); throw new Error('Picture processing is not supported on this device.'); }
  context.drawImage(bitmap, 0, 0, canvas.width, canvas.height);
  bitmap.close();

  let quality = 0.82;
  let blob = await canvasToBlob(canvas, quality);
  while (blob.size > MAX_COMPRESSED_SIZE_BYTES && quality > 0.58) {
    quality -= 0.08;
...
export function TechnicianJobView({ workOrderId }: { workOrderId: string }) {
  ...
  async function load() {
    try {
      const [job, items, jobPhotos] = await Promise.all([api.workOrder(workOrderId), api.workOrderChecklist(workOrderId), api.workOrderPhotos(workOrderId)]);
      setWorkOrder(job); setChecklist(items); setPhotos(jobPhotos); setError('');
  ...
  async function changeStatus(status: WorkOrderStatus) {
    setSaving(true);
    try { await api.changeWorkOrderStatus(workOrderId, { status }); await load(); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to update job status.'); }
    finally { setSaving(false); }
  }

  async function updateChecklist(item: WorkOrderChecklistItem, status: WorkOrderChecklistItem['status']) {
    try { await api.updateWorkOrderChecklistItem(workOrderId, item.id, { status }); setChecklist(await api.workOrderChecklist(workOrderId)); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to update checklist.'); }
  }
  ...
  async function uploadPhoto(event: ChangeEvent<HTMLInputElement>, category: WorkOrderPhoto['category']) {
    ...
    try {
      ...
      const storagePath = `${workOrderId}/${category.toLowerCase()}/${Date.now()}-${file.name}`;
      const { error: uploadError } = await supabase.storage.from(bucket).upload(storagePath, file, { contentType: 'image/webp', upsert: false });
      if (uploadError) throw uploadError;
      const { data } = supabase.storage.from(bucket).getPublicUrl(storagePath);
      try { await api.createWorkOrderPhoto(workOrderId, { category, url: data.publicUrl, storagePath, uploadedBy: session.user.email }); }
      catch (metadataError) { await supabase.storage.from(bucket).remove([storagePath]); throw metadataError; }
      setPhotos(await api.workOrderPhotos(workOrderId));
      event.target.value = '';
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to upload photo.'); }
  ...
  async function removePhoto(photo: WorkOrderPhoto) {
    ...
    try {
      ...
      setPhotos(await api.workOrderPhotos(workOrderId));
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to remove photo.'); }
  }
  ...
  const photoSection = (title: string, category: WorkOrderPhoto['category'], items: WorkOrderPhoto[]) => <section className="panel">
    ...
    <label>Add picture<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" disabled={uploading} onChange={(event) => void uploadPhoto(event, category)} /></label>
    <p>JPEG, PNG, or WebP only. Pictures are verified, resized to a maximum of 1920 pixels, and re-encoded as WebP before upload.</p>
