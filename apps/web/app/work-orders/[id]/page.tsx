...
export default async function TechnicianJobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppFrame active="/work-orders" email={appUser.email} user={appUser}><TechnicianJobView workOrderId={id} canAcknowledgeCompletion={appUser.role === 'ADMIN' || appUser.role === 'SUPERVISOR'} /></AppFrame>;
}
