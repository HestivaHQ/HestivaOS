...
type WorkOrderForm = { customerId: string; propertyId: string; technicianIds: string[]; jobLeaderId: string; crewId: string; serviceId: string; addOns: AddOnFormValue[]; frequency: WorkOrderFrequency | ''; customFrequencyNote: string; homeCondition: HomeCondition | ''; description: string; status: WorkOrder['status']; priority: WorkOrder['priority']; scheduledAt: string; completedAt: string };
const emptyForm: WorkOrderForm = { customerId: '', propertyId: '', technicianIds: [], jobLeaderId: '', crewId: '', serviceId: '', addOns: [], frequency: '', customFrequencyNote: '', homeCondition: '', description: '', status: 'NEW', priority: 'NORMAL', scheduledAt: '', completedAt: '' };
...
export function WorkOrdersManager({ createdById, createRoute = false, initialCustomerId, initialPropertyId }: { createdById: string; createRoute?: boolean; initialCustomerId?: string; initialPropertyId?: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const payload = { ...form, frequency: form.frequency || null, customFrequencyNote: form.frequency === 'CUSTOM' ? form.customFrequencyNote || null : null, homeCondition: form.homeCondition || null, technicianIds: form.technicianIds, jobLeaderId: form.jobLeaderId || null, crewId: form.crewId || null, scheduledAt: form.scheduledAt || undefined, completedAt: form.completedAt || undefined };
      if (editingId) {
        const { status, ...updates } = payload;
        const { technicianIds, jobLeaderId, crewId, ...workOrderUpdates } = updates;
        let updated = await api.updateWorkOrder(editingId, workOrderUpdates);
        updated = await api.assignWorkOrderTechnicians(editingId, { technicianIds, jobLeaderId, crewId });
