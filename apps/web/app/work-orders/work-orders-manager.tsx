  ...
  return <>
    ...
    <div className="resourceGrid">
      <form className="panel resourceForm" onSubmit={submit}>
        ...
        <h4>Property snapshot</h4>{(() => { const property = properties.find((item) => item.id === form.propertyId); return property ? <div className="propertySnapshot"><strong>{property.name}</strong><p>{[property.propertyTypeOption?.label, property.addressLine1, property.city].filter(Boolean).join(' · ')}</p>{property.accessNotes ? <p><strong>Access / household:</strong> {property.accessNotes}</p> : <p className="helpText">No access notes recorded on the property.</p>}</div> : <p className="helpText">Select a property to view its snapshot.</p>; })()}
        <h4>Visit</h4>
