  ...
  return <>
    ...
    <div className="resourceGrid">
      <form className="panel resourceForm" onSubmit={submit}>
        ...
        <fieldset><legend>Add-ons</legend><p className="helpText">Choose zero or more active add-on services.</p>{selectableAddOns.map((service) => <label className="checkboxLabel" key={service.id}><input type="checkbox" checked={form.addOnIds.includes(service.id)} onChange={(event) => setForm({ ...form, addOnIds: event.target.checked ? [...form.addOnIds, service.id] : form.addOnIds.filter((id) => id !== service.id) })} />{service.name}{service.status === 'INACTIVE' ? ' (inactive, historical)' : ''}</label>)}{!selectableAddOns.length ? <p className="helpText">No active add-ons are available.</p> : null}</fieldset>
