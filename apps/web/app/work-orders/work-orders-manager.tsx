...
type WorkOrderForm = { customerId: string; propertyId: string; technicianId: string; crewId: string; serviceId: string; addOnIds: string[]; frequency: WorkOrderFrequency | ''; customFrequencyNote: string; homeCondition: HomeCondition | ''; description: string; status: WorkOrder['status']; priority: WorkOrder['priority']; scheduledAt: string; completedAt: string };
const emptyForm: WorkOrderForm = { customerId: '', propertyId: '', technicianId: '', crewId: '', serviceId: '', addOnIds: [], frequency: '', customFrequencyNote: '', homeCondition: '', description: '', status: 'NEW', priority: 'NORMAL', scheduledAt: '', completedAt: '' };
const propertyCountLabels: Record<string, string> = { STUDIO: 'Studio', ONE: '1', TWO: '2', THREE: '3', FOUR: '4', FIVE_PLUS: '5+', FOUR_PLUS: '4+', THREE_PLUS: '3+' };
...
export function WorkOrdersManager({ createdById }: { createdById: string }) {
  ...
  function edit(workOrder: WorkOrder) {
    ...
    setForm({ customerId: workOrder.customerId, propertyId: workOrder.propertyId, technicianId: workOrder.technicianId ?? '', crewId: workOrder.crewId ?? '', serviceId: workOrder.serviceId ?? '', addOnIds: workOrder.addOns.map((item) => item.serviceId), frequency: workOrder.frequency ?? '', customFrequencyNote: workOrder.customFrequencyNote ?? '', homeCondition: workOrder.homeCondition ?? '', description: workOrder.description ?? '', status: workOrder.status, priority: workOrder.priority, scheduledAt: workOrder.scheduledAt?.slice(0, 16) ?? '', completedAt: workOrder.completedAt?.slice(0, 16) ?? '' });
  ...
  return <>
    ...
    <div className="resourceGrid">
      <form className="panel resourceForm" onSubmit={submit}>
        ...
        <h4>Job</h4>
        <label>Primary Service<select required value={form.serviceId} onChange={(e) => setForm({ ...form, serviceId: e.target.value })}><option value="">Select primary service</option>{[...primaryServices, ...(editingId && items.find((item) => item.id === editingId)?.service?.type === 'PRIMARY' && !primaryServices.some((service) => service.id === form.serviceId) ? [items.find((item) => item.id === editingId)!.service!] : [])].map((service) => <option key={service.id} value={service.id}>{service.name}{service.status === 'INACTIVE' ? ' (inactive, historical)' : ''}</option>)}</select></label>
        <fieldset className="addOnSection"><legend>Add-ons</legend><p className="helpText">Select any optional services for this job.</p>{selectableAddOns.length ? <div className="addOnGrid">{selectableAddOns.map((service) => { const inputId = `work-order-add-on-${service.id}`; const selected = form.addOnIds.includes(service.id); return <label className={`addOnOption${selected ? ' addOnOptionSelected' : ''}${service.status === 'INACTIVE' ? ' addOnOptionInactive' : ''}`} htmlFor={inputId} key={service.id}><input id={inputId} type="checkbox" checked={selected} onChange={(event) => setForm({ ...form, addOnIds: event.target.checked ? [...form.addOnIds, service.id] : form.addOnIds.filter((id) => id !== service.id) })} /><span>{service.name}{service.status === 'INACTIVE' ? <small>Inactive · historical selection</small> : null}</span></label>; })}</div> : <p className="addOnEmptyState">No add-ons are currently available.</p>}</fieldset>
        ...
        <h4>Property snapshot — current profile</h4>{(() => { const property = properties.find((item) => item.id === form.propertyId); if (!property) return <p className="helpText">Select a property to view its profile.</p>; const facts = [property.propertyTypeOption?.label, property.bedrooms && `${propertyCountLabels[property.bedrooms]} ${property.bedrooms === 'ONE' ? 'bedroom' : 'bedrooms'}`, property.bathrooms && `${propertyCountLabels[property.bathrooms]} ${property.bathrooms === 'ONE' ? 'bathroom' : 'bathrooms'}`, property.livingAreas && `${propertyCountLabels[property.livingAreas]} living areas`, property.storeys && `${propertyCountLabels[property.storeys]} ${property.storeys === 'ONE' ? 'storey' : 'storeys'}`, property.isEstateOrComplex ? 'Estate / complex' : null, property.hasPets ? 'Pets present' : null].filter(Boolean); const access = [property.requiresGateSecurityAccess ? 'Gate/security access required' : null, property.accessNotes, property.parkingNotes].filter(Boolean); const care = [property.petNotes, property.hasCameras ? 'Security cameras present' : null, property.offLimitsNotes, property.fragileItemNotes, property.productRestrictionNotes, property.allergyNotes].filter(Boolean); return <div className="propertySnapshot"><strong>{property.name}</strong><p>{[property.addressLine1, property.city].filter(Boolean).join(' · ')}</p>{facts.length ? <p>{facts.join(' · ')}</p> : null}{access.length ? <p><strong>Access:</strong> {access.join(' · ')}</p> : null}{care.length ? <p><strong>Care notes:</strong> {care.join(' · ')}</p> : null}{!access.length && !care.length ? <p className="helpText">No operational access or care notes recorded.</p> : null}</div>; })()}
        <h4>Visit</h4>
