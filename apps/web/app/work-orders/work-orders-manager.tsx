...
type WorkOrderForm = { customerId: string; propertyId: string; technicianIds: string[]; crewId: string; serviceId: string; addOns: AddOnFormValue[]; frequency: WorkOrderFrequency | ''; customFrequencyNote: string; homeCondition: HomeCondition | ''; description: string; status: WorkOrder['status']; priority: WorkOrder['priority']; scheduledAt: string; completedAt: string };
const emptyForm: WorkOrderForm = { customerId: '', propertyId: '', technicianIds: [], crewId: '', serviceId: '', addOns: [], frequency: '', customFrequencyNote: '', homeCondition: '', description: '', status: 'NEW', priority: 'NORMAL', scheduledAt: '', completedAt: '' };
...
export function WorkOrdersManager({ createdById }: { createdById: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const payload = { ...form, frequency: form.frequency || null, customFrequencyNote: form.frequency === 'CUSTOM' ? form.customFrequencyNote || null : null, homeCondition: form.homeCondition || null, technicianIds: form.technicianIds, crewId: form.crewId || null, scheduledAt: form.scheduledAt || undefined, completedAt: form.completedAt || undefined };
      if (editingId) {
        const { status, ...updates } = payload;
        const { technicianIds, crewId, ...workOrderUpdates } = updates;
        let updated = await api.updateWorkOrder(editingId, workOrderUpdates);
        updated = await api.assignWorkOrderTechnicians(editingId, { technicianIds, crewId });
