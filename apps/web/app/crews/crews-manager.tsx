...
export function CrewsManager() {
  ...
  function toggleMember(id: string) {
    ...
    setForm({ ...form, memberIds, leaderId: memberIds.includes(form.leaderId) ? form.leaderId : '' });
  }
  ...
  return <>
    ...
    <div className="resourceGrid">
      <form className="panel resourceForm" onSubmit={submit}>
        ...
        <label>Crew leader<select value={form.leaderId} onChange={(e) => setForm({ ...form, leaderId: e.target.value })}><option value="">No leader</option>{selectedMembers.map((technician) => <option key={technician.id} value={technician.id}>{technician.firstName} {technician.lastName}</option>)}</select></label>
      ...
      <section className="panel"><div className="panelHeader"><h3>Crew list</h3><input className="searchInput" placeholder="Search crews" value={search} onChange={(e) => setSearch(e.target.value)} /></div><div className="dataList">
        {items.map((crew) => <article className="dataRow" key={crew.id}><div><strong>{crew.name}</strong><p>{crew.leader ? `Leader: ${crew.leader.firstName} ${crew.leader.lastName}` : 'No leader'} · {crew.members.length} member{crew.members.length === 1 ? '' : 's'} · {crew._count?.workOrders ?? 0} work orders</p><p>{crew.members.map((member) => `${member.technician.firstName} ${member.technician.lastName}`).join(' · ') || 'No members assigned'}</p></div><div className="rowActions"><span className="statusPill">{crew.status}</span><button onClick={() => edit(crew)}>Edit</button><button className="dangerButton" onClick={() => void remove(crew.id)}>Delete</button></div></article>)}
