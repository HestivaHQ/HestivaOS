...
export function ServicesCatalogue() {
  ...
  return <>
    ...
    <section className="panel"><div className="panelHeader serviceCatalogueHeader"><h3>Available services</h3><label>Search<input className="searchInput" type="search" value={search} onChange={(event) => setSearch(event.target.value)} /></label></div><div className="dataList">
      {items.map((service) => <article className="dataRow" key={service.id}><div><strong>{service.name}</strong><p>{service.description || 'No operational description'}</p></div><span className="statusPill">{service.type === 'ADD_ON' ? 'ADD-ON' : 'PRIMARY'}</span></article>)}
