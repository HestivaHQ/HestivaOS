...
export default async function HomePage() {
  ...
  const statusCards: Array<{ status: WorkOrderStatus; label: string; value: number }> = Object.entries(dashboard.statusBreakdown)
    .filter(([status]) => !['CLOSED', 'CANCELLED'].includes(status))
    .map(([status, value]) => ({ status: status as WorkOrderStatus, label: readableStatus(status), value }));
  ...
  return (
    <main className="appShell">
      <aside className="sidebar">
        ...
        <div className="accountBlock"><span>{user?.email ?? 'Authenticated user'}</span><SignOutButton /></div>
      </aside>

      <section className="content">
        <header className="pageHeader">
          <div><p className="eyebrow">Live overview</p><h2>Dashboard</h2><p>Customers, properties, and maintenance work at a glance.</p></div>
          <span className={`systemBadge ${dashboard.available ? 'online' : 'offline'}`}>{dashboard.available ? 'API connected' : 'API unavailable'}</span>
        </header>
        ...
        <section className="panel">
          ...
          <div className="metricGrid">{statusCards.map((status) => <Link className="metricCard" href={`/work-orders?status=${status.status}`} key={status.status}><span>{status.label}</span><strong>{status.value}</strong></Link>)}</div>
        </section>

        <section className="panel">
          <div className="panelHeader"><div><p className="eyebrow">Technician workload</p><h3>Active assignments and today&apos;s scheduled jobs</h3></div></div>
          {dashboard.technicianWorkload.length ? (
            <div className="technicianWorkloadList">{dashboard.technicianWorkload.map((technician) => (
              <article className="technicianWorkloadItem" key={technician.technicianId}>
                <strong>{technician.technicianName}</strong>
                <dl><div><dt>Active work orders</dt><dd>{technician.activeWorkOrderCount}</dd></div><div><dt>Scheduled today</dt><dd>{technician.scheduledTodayCount}</dd></div><div><dt>High priority</dt><dd>{technician.highPriorityCount}</dd></div></dl>
              </article>
            ))}</div>
        ...
        <section className="panel">
          <div className="panelHeader"><div><p className="eyebrow">Alerts</p><h3>Action required</h3></div></div>
          {(() => {
            const alerts = [
        ...
        <section className="panel">
          <div className="panelHeader"><div><p className="eyebrow">Today&apos;s schedule</p><h3>Scheduled work orders</h3></div><Link href="/work-orders">View all</Link></div>
          {dashboard.todayScheduledWorkOrders.length ? (
            <div className="workList">{dashboard.todayScheduledWorkOrders.map((workOrder) => (
              <Link className="workItem scheduledWorkItem" href={`/work-orders?edit=${workOrder.id}`} key={workOrder.id}>
                <div className="scheduledWorkDetails"><strong>{workOrder.customer.name}</strong><p>{workOrder.property.name}</p><dl><div><dt>Assigned technician</dt><dd>{workOrder.technician ? `${workOrder.technician.firstName} ${workOrder.technician.lastName}` : 'Unassigned'}</dd></div><div><dt>Scheduled time</dt><dd><time dateTime={workOrder.scheduledAt ?? undefined}>{formatTime(workOrder.scheduledAt)}</time></dd></div><div><dt>Priority</dt><dd>{workOrder.priority}</dd></div></dl></div>
        ...
        <section className="panel">
          <div className="panelHeader"><div><p className="eyebrow">Upcoming work</p><h3>Scheduled for the next 7 days</h3></div><Link href="/work-orders">View all</Link></div>
          {dashboard.upcomingScheduledWorkOrders.length ? (
            <div className="workList">{dashboard.upcomingScheduledWorkOrders.map((workOrder) => (
              <Link className="workItem scheduledWorkItem" href={`/work-orders?edit=${workOrder.id}`} key={workOrder.id}>
                <div className="scheduledWorkDetails"><strong>{workOrder.customer.name}</strong><p>{workOrder.property.name}</p><dl><div><dt>Date</dt><dd><time dateTime={workOrder.scheduledAt ?? undefined}>{formatDate(workOrder.scheduledAt)}</time></dd></div><div><dt>Assigned technician</dt><dd>{workOrder.technician ? `${workOrder.technician.firstName} ${workOrder.technician.lastName}` : 'Unassigned'}</dd></div><div><dt>Priority</dt><dd>{workOrder.priority}</dd></div></dl></div>
                <div className="workMeta"><span className="statusPill">{readableStatus(workOrder.status)}</span></div>
              </Link>
            ))}</div>
        ...
        <section className="panel">
          <div className="panelHeader"><div><p className="eyebrow">Recent activity</p><h3>Latest work-order updates</h3></div><Link href="/work-orders">View all</Link></div>
          {dashboard.recentWorkOrderActivities.length ? (
            <div className="recentActivityList" role="list">{dashboard.recentWorkOrderActivities.map((activity) => (
