  ...
  return <>
    ...
    <div className="resourceGrid">
      {editorOpen ? <form className="panel resourceForm" onSubmit={submit}>
        ...
        <label>Crew<select aria-label="Crew" value={form.crewId} onChange={(event) => { setTechnicianSearch(''); setForm({ ...form, crewId: event.target.value, technicianId: '' }); }}><option value="">No crew</option>{crews.filter((crew) => crew.status === 'ACTIVE' || crew.id === form.crewId).map((crew) => <option key={crew.id} value={crew.id}>{crew.name}{crew.status === 'INACTIVE' ? ' (inactive, historical)' : ''}</option>)}</select></label>
        ...
        <label>Work order<select aria-label="Work order" value={form.workOrderId} onChange={(event) => setForm({ ...form, workOrderId: event.target.value })}><option value="">No linked work order</option>{workOrders.map((workOrder) => <option key={workOrder.id} value={workOrder.id}>{workOrderReference(workOrder)} · {workOrderDisplayLabel(workOrder)}</option>)}</select></label>
