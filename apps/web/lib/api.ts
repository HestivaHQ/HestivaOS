...
export type WorkOrder = { id: string; customerId: string; propertyId: string; createdById: string; technicianId: string | null; jobLeaderId: string | null; jobLeader: Technician | null; assignedTechnicians: Array<{ technicianId: string; technician: Technician }>; crewId: string | null; serviceId: string | null; reference: string | null; title: string; service: Service | null; addOns: Array<{ serviceId: string; quantity: number; service: Service }>; frequency: WorkOrderFrequency | null; customFrequencyNote: string | null; homeCondition: HomeCondition | null; description?: string | null; status: WorkOrderStatus; priority: 'LOW' | 'NORMAL' | 'HIGH' | 'URGENT'; scheduledAt: string | null; completedAt?: string | null; createdAt: string; customer: Customer; property: Property; technician: Technician | null; crew: Crew | null };
...
export type WorkOrderActivity = { id: string; type: 'WORK_ORDER_CREATED' | 'STATUS_CHANGED' | 'TECHNICIAN_ASSIGNED' | 'TECHNICIAN_CHANGED' | 'TECHNICIAN_REMOVED' | 'CREW_ASSIGNED' | 'CREW_CHANGED' | 'CREW_REMOVED' | 'JOB_LEADER_CHANGED' | 'WORK_ORDER_CLOSED' | 'WORK_ORDER_CANCELLED'; previousStatus: WorkOrderStatus | null; newStatus: WorkOrderStatus | null; note: string | null; actor: AppUser | null; createdAt: string };
...
export const api = {
  ...
  assignWorkOrderTechnicians: (id: string, input: { technicianIds: string[]; crewId?: string | null; jobLeaderId?: string | null }) => apiFetch<WorkOrder>(`/work-orders/${id}/assignment`, { method: 'PATCH', ...json(input) }),
