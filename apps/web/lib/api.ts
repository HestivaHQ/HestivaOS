...
export type FloorSize = 'UNDER_80' | 'FROM_80_TO_150' | 'FROM_151_TO_250' | 'OVER_250' | 'UNKNOWN';
...
export type WorkOrder = { id: string; customerId: string; propertyId: string; createdById: string; technicianId: string | null; crewId: string | null; serviceId: string | null; reference: string | null; title: string; service: Service | null; addOns: Array<{ serviceId: string; service: Service }>; frequency: WorkOrderFrequency | null; customFrequencyNote: string | null; homeCondition: HomeCondition | null; description?: string | null; status: WorkOrderStatus; priority: 'LOW' | 'NORMAL' | 'HIGH' | 'URGENT'; scheduledAt: string | null; completedAt?: string | null; createdAt: string; customer: Customer; property: Property; technician: Technician | null; crew: Crew | null };
export type RecurringServiceAgreement = { id: string; propertyId: string; serviceId: string; frequency: Exclude<WorkOrderFrequency, 'ONE_TIME'>; status: RecurringServiceStatus; effectiveDate: string; endDate: string | null; weekday: RecurrenceWeekday | null; dayOfMonth: number | null; preferredTimeWindow: PreferredTimeWindow | null; customFrequencyNote: string | null; recurringInstructions: string | null; nextServiceDate: string | null; property: Property & { customer: Customer }; service: Service; addOns: Array<{ serviceId: string; service: Service }>; _count: { workOrders: number } };
export type RecurringServiceInput = { propertyId: string; serviceId: string; addOnIds: string[]; frequency: Exclude<WorkOrderFrequency, 'ONE_TIME'>; effectiveDate: string; endDate?: string | null; weekday?: RecurrenceWeekday | null; dayOfMonth?: number | null; preferredTimeWindow?: PreferredTimeWindow | null; customFrequencyNote?: string | null; recurringInstructions?: string | null };
...
export type WorkOrderInput = { customerId: string; propertyId: string; createdById: string; technicianId?: string | null; crewId?: string | null; serviceId: string; addOnIds?: string[]; frequency?: WorkOrderFrequency | null; customFrequencyNote?: string | null; homeCondition?: HomeCondition | null; description?: string; status?: WorkOrder['status']; priority?: WorkOrder['priority']; scheduledAt?: string; completedAt?: string };
