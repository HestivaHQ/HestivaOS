...
test('quote domain has a stable reference, commercial lifecycle, validity, and revision history', () => {
  assert.match(schema, /enum QuoteStatus \{[\s\S]*SUBMITTED[\s\S]*ACCEPTED[\s\S]*DECLINED[\s\S]*EXPIRED[\s\S]*NEEDS_ATTENTION[\s\S]*\}/);
  assert.match(schema, /model Quote \{[\s\S]*reference String @unique[\s\S]*currentRevisionNumber Int @default\(1\)[\s\S]*validUntil DateTime/);
...
test('quote photos preserve provenance and transfer status without duplicating work-order photo ownership', () => {
  ...
  assert.match(schema, /model QuotePhoto \{[\s\S]*quoteId String[\s\S]*quoteRevisionId String\?[\s\S]*source QuotePhotoSource[\s\S]*status QuotePhotoStatus/);
});

test('migration creates the quote aggregate and atomic daily reference counter', () => {
