...
- Manual review found one documentation statement that still described the pre-hardening equal-length comparison. Corrected it before merge review so the runtime-support document now matches the SHA-256-digest implementation. Earlier complete CI heads passed the full GitHub PR quality gate; this historical log/changelog reconciliation and append-only-history correction are included before the final-head quality-gate run and merge decision.
