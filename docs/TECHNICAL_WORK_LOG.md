...
- Converted the Admin Settings User Access signpost into `/admin/settings/user-access`, an ADMIN-only server-rendered entry with a responsive client manager. It lists application-user name, email, role, explicit OS access state only. It contains no employee-record fields, and last authenticated activity is labelled unavailable because no verified source is stored.
