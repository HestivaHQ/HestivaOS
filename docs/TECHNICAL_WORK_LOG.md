...
- Manual review found one documentation statement that still described the pre-hardening equal-length comparison. Corrected it before merge review so the runtime-support document now matches the SHA-256-digest implementation. The first complete CI head and the documentation-correction head both passed the full GitHub PR quality gate; this historical log/changelog reconciliation is the final repository-policy correction before a fresh final-head run and merge decision.
...
- Restricted POST/PATCH catalogue management to ADMIN and removed permanent deletion from the contract and UI. Operational reads remain authenticated. Admin Settings now owns searchable create/edit/deactivate/reactivate controls; `/services` lists active records. Template assignment continues to reject inactive records while existing inactive historical relationships remain included and readable.
