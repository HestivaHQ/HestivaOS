...
- `META_WHATSAPP_BUSINESS_ACCOUNT_ID` — Meta WhatsApp Business Account ID used by ADMIN-only WhatsApp Business management operations such as listing the account's message templates. Configure the WABA that owns the currently configured phone-number ID; do not substitute an unrelated or production WABA while the runtime is deliberately pointed at Meta's test account.
...
The ADMIN-only WhatsApp Business operations surface reuses the same server-side access token and explicit Graph API version. Listing message templates additionally requires `META_WHATSAPP_BUSINESS_ACCOUNT_ID`; sending an approved template uses the configured `META_WHATSAPP_PHONE_NUMBER_ID`. Provider credentials remain server-side and are never returned to browser code.
