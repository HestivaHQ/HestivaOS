...
- `Quote` as the stable commercial identity, with unique public reference, commercial status, 30-day validity boundary, current revision number, and nullable future operational-link identifiers;
...
- `QuotePhoto` as durable quote evidence/provenance with customer/Admin source and transfer status;
