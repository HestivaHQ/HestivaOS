...
- Next.js reports the retained middleware convention as deprecated until the separate proxy migration is completed.
...
Review if OpenNext or Cloudflare reports a Next.js 16/Turbopack incompatibility, if a verified vulnerability affects 16.3.0 or its resolved graph, or when the middleware-to-proxy migration is undertaken.
