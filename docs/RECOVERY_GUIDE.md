...
## Quote Send + Tracking Reliability + Recovery V1 (2026-08-24)

Treat an email delivery attempt that remains `PENDING` after a network failure or lost Resend HTTP response as **uncertain**, not failed. The absence of a trustworthy synchronous response does not prove whether Resend accepted the submission. While that original attempt remains genuinely uncertain, do not press Send again, create a replacement Correspondence attempt, issue another customer capability, manually mark the attempt failed, or use provider dashboard observations as a substitute for authenticated evidence.

An ADMIN may use the Quote Send / Share reconciliation action for the exact current Quote revision. Reconciliation operates on the original pending Correspondence attempt and does not send another email or rotate the secure customer capability. It first revalidates the exact revision, then inspects authenticated append-only Resend provider events correlated to that attempt. If signed provider evidence proves Resend accepted/processed the original submission, reconciliation records the original Correspondence attempt as provider-accepted. If signed evidence proves a provider submission failure and no stronger acceptance evidence exists, reconciliation records the original attempt failed. If neither conclusion is defensible, the attempt remains pending and new email preparation stays blocked.

Keep provider acceptance separate from downstream delivery outcome. A Resend-side email object/event proves provider processing; it does not by itself prove recipient delivery or customer view. `email.sent`, `email.delivered`, `email.delivery_delayed`, `email.bounced`, and `email.complained` prove that the original submission reached provider processing even when the later lifecycle is unsuccessful. A bounce is recipient-mail-server rejection after provider acceptance; a complaint is a delivered message later reported as spam; delayed means delivery remains temporarily delayed. `email.failed` and `email.suppressed` are failure lifecycle evidence, but must not retroactively rewrite stronger evidence that the original API submission was accepted. Staff must not describe downstream bounce, complaint, suppression, or later failure as an original API rejection when provider acceptance is already established.

Resend webhooks may be duplicated, delayed, or arrive out of order. Preserve provider-event evidence append-only, deduplicate by authenticated provider event identity, and use provider occurrence timestamps rather than arrival order. Do not delete or reorder evidence to force a preferred status. `email.opened` and `email.clicked` remain excluded from Quote customer-view evidence; only the secure Quote `VIEW_CONFIRMED` path proves an operational customer view.

Customer ACCEPT/DECLINE is independent of email transport recovery. If the customer responds while the email attempt is still uncertain, preserve the existing exact-revision response evidence and canonical Quote lifecycle result. Reconciliation may still finish the historical email attempt after the Quote has become `ACCEPTED` or `DECLINED`, provided it is still the same exact current revision; do not require the Quote to return to `SUBMITTED`, ask the customer to respond again, or create a second acceptance/decline path. Existing same-decision/idempotency replay remains the recovery mechanism for a transient canonical conversion failure.

Never resurrect customer access during transport recovery. If the original grant is revoked, expired, superseded, stale for the current revision, or otherwise inactive, reconciliation fails closed rather than reactivating or reconstructing it. The raw bearer capability is intentionally not persisted and must never be recovered from logs, Correspondence evidence, webhook data, incident notes, or provider metadata. Once the original uncertain attempt has a defensible terminal Correspondence outcome, a deliberate new send is allowed only through the ordinary authenticated ADMIN Send path and normal current-Quote eligibility checks; that new preparation creates its own Correspondence attempt and exact-revision capability. A downstream bounce/failure/suppression may therefore permit a deliberate fresh resend after the original attempt is truthfully reconciled, while a still-uncertain attempt may not.

During an incident, preserve the Quote, exact revision, Correspondence record/attempt, provider-event evidence, customer response evidence, and access-grant history. Do not edit delivery rows directly, manufacture provider references, replay an unknown raw bearer URL, weaken ADMIN authorization, treat WhatsApp composer-open evidence as sent/delivered/read evidence, or infer `VIEW_CONFIRMED` from Resend engagement events. Use the ADMIN reconciliation control and existing bounded evidence/status surfaces; investigate unmatched signed events without manually relinking them across attempts.
