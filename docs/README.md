...
Final PR validation follows the repository's **frozen-head verification workflow**. Complete implementation, focused tests, mandatory documentation and coordination updates before the final validation cycle; audit the complete diff; then freeze the exact head and run required CI. Do not create new heads for speculative cleanup or wording polish while final CI is running. A CI failure, review/security finding, materially changed merge base, or maintainer correction is new evidence that reopens the head: diagnose the evidenced cause, make the smallest justified fix on the same branch, re-audit the affected area plus complete-diff integrity, freeze the new head, and rerun the full required gates. A green result applies only to the exact tested head and materially unchanged integration base. This improves throughput by reducing avoidable CI restarts without weakening any safety or documentation gate.
