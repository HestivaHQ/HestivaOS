...
When LR-1B exposes a repeatable failure, the full suite remains the acceptance gate but should not be used as the primary debugger. Follow `LR1B_DIAGNOSTIC_STRATEGY.md` and ADR-0095: isolate the smallest real product path that reproduces the failure, retain the same safety boundary and real authenticated UI where applicable, emit only bounded non-secret diagnostics, prove the failing boundary, then fix product code and rerun the affected diagnostic before returning to the full acceptance lane.
