...
2. **Messenger provider edge:** authenticated Page webhook verification/signature handling, normalized inbound persistence, Page-token-gated standard-window TEXT replies, 24-hour same-conversation eligibility, and fail-closed unknown-outcome replay blocking are implemented. Richer media/status fidelity and any out-of-window/message-tag policy remain separate work.
3. **Provider identity ↔ Customer linking:** deliberate ADMIN-only linking to an existing canonical Customer is implemented using the existing nullable conversation relation. Same-Customer replay is idempotent; fuzzy automatic matching, unlinking and silent reassignment remain disabled.
