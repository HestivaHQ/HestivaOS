...
## Why completed execution correction is append-only and scoped

A genuine field-record mistake must be repairable, but reopening the Work Order lifecycle or overwriting an outcome would obscure what was originally submitted and reviewed. A management authorization scoped to named frozen sections, followed by Technician-owned append-only outcomes and fresh acknowledgement, separates correction authority from field truth while retaining every prior fact.
