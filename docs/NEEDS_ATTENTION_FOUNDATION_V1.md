...
## Phase 4A incident producer (2026-08-19)

Each unresolved `WorkOrderIncident` contributes one stable `work-order-incident:<incident-id>:review` Management Review condition. Safety-critical incidents are Critical and other controlled incident categories are High. Incident resolution clears the authoritative condition; explicit reopen reactivates the same item and occurrence history. The action deep-links to the Work Order incident review surface. No parallel incident inbox or notification delivery exists.
