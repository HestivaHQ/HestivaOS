...
| Shifts | `shifts-manager` | Crew; technician; work order | searchable select controls | canonical IDs | RELATIONSHIP (3) | Bounded server-backed search; Crew member snapshot for designated Technician; selected history retained | **Yes—Phase 1B Shift Planning searchability** | Stores canonical IDs and avoids fixed 100-record reference snapshots without changing scheduling or staffing rules. |
