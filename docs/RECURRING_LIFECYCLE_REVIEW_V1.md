...
Each due transition uses a conditional database update that still requires the agreement to be PAUSED and due. This is the idempotency/concurrency boundary: if more than one API process or runner observes the same row, only one update succeeds. The winning transition clears `autoResumeDate` and recalculates `nextServiceDate` from the current business date. If the agreement end date has already passed, it becomes ENDED instead and `nextServiceDate` is cleared.
