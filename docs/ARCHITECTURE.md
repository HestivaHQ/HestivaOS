...
## Persistent authenticated navigation boundary (2026-08-28)

The Homent office application uses one Next.js authenticated route-group layout as the persistent desktop/mobile/account shell. Middleware verifies asymmetric Supabase access-token claims through `auth.getClaims()` rather than performing routine remote `getUser()` calls; verification errors and missing configuration fail protected routes closed, while Supabase cookie updates continue to be copied to the response. The separately scoped Homent Technician layout, login/auth callback, and public Quote surface remain outside the office shell.

The authenticated layout obtains the canonical application User through read-only `GET /api/v1/users/me`. The API global guard still verifies the bearer token locally against Supabase JWKS, resolves the HestivaOS User, requires ACTIVE status, and applies route role metadata. `GET /users/me` returns that guard-resolved User without a second lookup. `POST /users/sync` remains available only as the deliberate post-login identity bootstrap/reconciliation path described below; routine navigation never uses it. Provider claims are not used as application roles.

Office pages provide nested route content rather than constructing `AppFrame`; a shared loading boundary streams inside the persistent shell. Cookie access at the authenticated layout remains dynamic, but the root no longer blanket-forces all routes dynamic. See ADR-0091 and `PERFORMANCE_AUDIT.md`.
...
Final gates are not skipped by changed-file paths. This preserves repository-wide integration confidence while reducing wall-clock latency through safe parallelism and removal of duplicated compilation. The documentation validator checks the PR impact declaration and mechanically determinable companions; Changelog significance and the frozen Technical Work Log follow ADR-0069 and `DOCUMENTATION_AUTHORITY.md`. `AGENTS.md` remains authoritative for the complete change-to-document matrix.
...
- **Frontend:** `@hestiva/web` is Next.js 16.3.4 rendered by the Cloudflare Worker `hestivaos` through OpenNext. Cloudflare owns web build execution, edge runtime, static assets, and web observability.
...
The web application displays the Supabase-authenticated email read-only and sends password changes directly to the installed Supabase client through `auth.updateUser({ password })`; passwords never pass through or persist in the Hestiva API. The canonical `/admin/settings` route reads the authenticated application User from the shared read-only current-user boundary and renders only when that record has the exact `ADMIN` role. All other roles redirect to the dashboard. Its User Access and Business Profile cards link to their implemented management routes.
...
`GET` and `PATCH /api/v1/admin/business-profile` require the exact `ADMIN` role through controller metadata and the global authenticated-user guard. The web route also checks the read-only current application role before reading. Updates use an explicit allowlist and return neither the singleton key nor timestamps. Mutation logs contain the actor identifier and changed field names only; persistent audit history is deferred because the repository has no appropriate general audit model.
