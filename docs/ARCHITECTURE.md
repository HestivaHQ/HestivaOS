...
Agreement and initial visit preserve primary Service, frequency, supported instructions, and exact generic/Laundry/Ironing add-on quantities; the visit also preserves home condition. Preferred time uses the agreement's controlled time-window destination, not a fabricated Work Order timestamp. Accepted pricing and remaining source-only visit context stay on the immutable revision. Review UI and the remaining non-lossy handoff are not implemented. Website ingestion and ordinary recurring CRUD/generation are unchanged.
...
The Work Order projects primary Service, ONE_TIME frequency, home condition, preferred service date, supported visit description, and all canonical add-ons with exact positive quantities, including Laundry and Ironing loads. New Properties receive the established address/profile/household/care fields; existing canonical records are not overwritten. Pricing remains on the accepted revision because Work Order has no commercial snapshot destination. Preferred time window, exact floor, building access, access/key/parking details, Quote-photo linkage, eco preference, flexibility/urgency, and remaining source-only instructions are not projected without an authoritative non-lossy destination. Supported recurring Quotes use the recurring agreement plus initial-visit path described above; ONE_TIME creates no `RecurringServiceAgreement`.
