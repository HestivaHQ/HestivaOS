...
The web application displays the Supabase-authenticated email read-only and sends password changes directly to the installed Supabase client through `auth.updateUser({ password })`; passwords never pass through or persist in the Hestiva API. The canonical `/admin/settings` route synchronizes the authenticated application User on the server and renders only when that record has the exact `ADMIN` role. All other roles redirect to the dashboard. Its User Access card links to the implemented management route; Business Profile remains a non-destructive Slice 4 signpost.
